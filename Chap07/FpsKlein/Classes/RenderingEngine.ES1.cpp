...
namespace ES1 {
    ...
    void RenderingEngine::Initialize()
    {
        ...
        m_offscreenSize = vec2(width, height);
    ...
    void RenderingEngine::Render(float objectTheta, float fboTheta) const
    {
        ...
        const int JitterCount = 2;
        const vec2* JitterOffsets = JitterOffsets2;
        ...
        for (int i = 0; i < JitterCount; i++) {
            ...
            glViewport(0, 0, m_offscreenSize.x, m_offscreenSize.y);
            glBindFramebufferOES(GL_FRAMEBUFFER_OES, m_framebuffers.Scene);
            glBindRenderbufferOES(GL_RENDERBUFFER_OES, m_renderbuffers.SceneColor);
    ...
    void RenderingEngine::RenderPass(float objectTheta, float fboTheta, vec2 offset) const
    {
        ...
        const float AspectRatio = (float) m_screenSize.y / m_screenSize.x;
        ...
        offset.x *= (RightPlane - LeftPlane) / m_offscreenSize.x;
        offset.y *= (BottomPlane - TopPlane) / m_offscreenSize.y;
